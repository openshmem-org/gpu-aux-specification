...
Applications that do not utilize GPU-attached memory buffers or the interfaces
...
\item Query interfaces for discovering implementation capabilities and symmetric
...
specification. Each \ac{PE} that utilizes the GPU support defined by this
...
have a valid PE-to-GPU association to use these interfaces.
...
support. When this option is provided, the implementation shall establish the
PE-to-GPU association and prepare the runtime for GPU-aware operations on the
\ac{GPU} symmetric heap. An implementation may support
...
runtime for communication operations invoked from within GPU kernels. An
...
\CONST{SHMEM\_DEVICE\_TEAM\_WORLD} shall be a subset of or equal to
...
allows users to configure GPU-related aspects of the \openshmem implementation.
...
The routines in this section provide initialization, memory management, and
query operations on the \ac{GPU} symmetric heap.
...
\VAR{requested} argument. When \CONST{SHMEM\_DEVICE\_HOST\_INIT} is included in
\VAR{requested}, the implementation shall establish the PE-to-GPU association and
initialize the \ac{GPU} symmetric heap for GPU-aware communication. When
\CONST{SHMEM\_DEVICE\_KERNEL\_INIT} is included, the implementation shall
additionally prepare the runtime for GPU-centric communication operations
invoked from within GPU kernels.
...
The \openshmem \ac{RMA}, \ac{AMO}, signaling, and collective routines defined in
\openshmem 1.6 may be used with GPU-attached memory buffers as operands. The
routine interfaces are unchanged. The tables below specify the allowed memory
locations for each argument.
...
Routines: \FUNC{shmem\_put}, \FUNC{shmem\_\TYPENAME\_put},
\FUNC{shmem\_put\SIZE}, \FUNC{shmem\_putmem}, and their \texttt{shmem\_ctx\_*}
variants.
...
Routines: \FUNC{shmem\_p}, \FUNC{shmem\_\TYPENAME\_p}, and their
\texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_iput}, \FUNC{shmem\_\TYPENAME\_iput},
\FUNC{shmem\_iput\SIZE}, and their \texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_put\_nbi}, \FUNC{shmem\_\TYPENAME\_put\_nbi},
\FUNC{shmem\_put\SIZE\_nbi}, \FUNC{shmem\_putmem\_nbi}, and their
\texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_get}, \FUNC{shmem\_\TYPENAME\_get},
\FUNC{shmem\_get\SIZE}, \FUNC{shmem\_getmem}, and their \texttt{shmem\_ctx\_*}
variants.
...
Routines: \FUNC{shmem\_g}, \FUNC{shmem\_\TYPENAME\_g}, and their
\texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_iget}, \FUNC{shmem\_\TYPENAME\_iget},
\FUNC{shmem\_iget\SIZE}, and their \texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_get\_nbi}, \FUNC{shmem\_\TYPENAME\_get\_nbi},
\FUNC{shmem\_get\SIZE\_nbi}, \FUNC{shmem\_getmem\_nbi}, and their
\texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_atomic\_set}, \FUNC{shmem\_atomic\_add},
\FUNC{shmem\_atomic\_inc}, \FUNC{shmem\_atomic\_and},
\FUNC{shmem\_atomic\_or}, \FUNC{shmem\_atomic\_xor}, and their
\texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_atomic\_fetch}, \FUNC{shmem\_atomic\_swap},
\FUNC{shmem\_atomic\_compare\_swap}, \FUNC{shmem\_atomic\_fetch\_add},
\FUNC{shmem\_atomic\_fetch\_inc}, \FUNC{shmem\_atomic\_fetch\_and},
\FUNC{shmem\_atomic\_fetch\_or}, \FUNC{shmem\_atomic\_fetch\_xor}, and their
\texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_atomic\_fetch\_nbi}, \FUNC{shmem\_atomic\_swap\_nbi},
\FUNC{shmem\_atomic\_compare\_swap\_nbi},
\FUNC{shmem\_atomic\_fetch\_add\_nbi},
\FUNC{shmem\_atomic\_fetch\_inc\_nbi},
\FUNC{shmem\_atomic\_fetch\_and\_nbi},
\FUNC{shmem\_atomic\_fetch\_or\_nbi},
\FUNC{shmem\_atomic\_fetch\_xor\_nbi}, and their \texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_put\_signal}, \FUNC{shmem\_\TYPENAME\_put\_signal},
\FUNC{shmem\_put\SIZE\_signal}, \FUNC{shmem\_putmem\_signal}, and their
\texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_put\_signal\_nbi},
\FUNC{shmem\_\TYPENAME\_put\_signal\_nbi},
\FUNC{shmem\_put\SIZE\_signal\_nbi}, \FUNC{shmem\_putmem\_signal\_nbi}, and
their \texttt{shmem\_ctx\_*} variants.
...
Routine: \FUNC{shmem\_signal\_fetch}.
...
Routines: \FUNC{shmem\_wait\_until}, \FUNC{shmem\_\TYPENAME\_wait\_until}, and
their \texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_wait\_until\_all}, \FUNC{shmem\_wait\_until\_any},
\FUNC{shmem\_wait\_until\_some}, and their vector variants.
...
Routines: \FUNC{shmem\_test}, \FUNC{shmem\_\TYPENAME\_test}, and their
\texttt{shmem\_ctx\_*} variants.
...
Routines: \FUNC{shmem\_test\_all}, \FUNC{shmem\_test\_any},
\FUNC{shmem\_test\_some}, and their vector variants.
...
Routine: \FUNC{shmem\_signal\_wait\_until}.
...
operations on symmetric data objects when GPU-aware communication is enabled.
...
operations on symmetric data objects issued to a particular \ac{PE} on the given
context prior to the call to \FUNC{shmem\_fence} are guaranteed to be delivered
before any subsequent such operations to the same \ac{PE} on the same context.
...
operations on symmetric data objects which makes the updates visible to all other
...
\subsubsection{SHMEM\_PUT\_SIGNAL (Cross-Domain Ordering)}\label{subsec:ga_put_signal_ordering}
...
Routines: \FUNC{shmem\_barrier\_all}, \FUNC{shmem\_barrier}.
...
kernels. These routines shall not be called from the host.
...
or teams created from \CONST{SHMEM\_DEVICE\_TEAM\_WORLD} through
\FUNC{shmem\_team\_split\_strided} and \FUNC{shmem\_team\_split\_2d} routines
are capable of performing GPU-centric communication operations from within
\ac{GPU} kernels. Using a context associated with any other team for GPU-centric
operations results in \textbf{undefined behavior}.

\CONST{SHMEM\_DEVICE\_CTX\_DEFAULT} is a predefined communication context of
type \CTYPE{shmem\_ctx\_t} associated with \CONST{SHMEM\_DEVICE\_TEAM\_WORLD}.
All GPU-centric \ac{RMA} and \ac{AMO} operations that do not specify a context
shall be performed on \CONST{SHMEM\_DEVICE\_CTX\_DEFAULT}. \ac{GPU} threads may
pass \CONST{SHMEM\_DEVICE\_CTX\_DEFAULT} to any device-callable routine that
accepts a context argument.

Explicit device contexts are created on the host using
\FUNC{shmem\_team\_create\_ctx} with the \CONST{SHMEM\_DEVICE\_CTX} option. The
team argument shall be \CONST{SHMEM\_DEVICE\_TEAM\_WORLD} or a team derived from
\CONST{SHMEM\_DEVICE\_TEAM\_WORLD}. Once created, these contexts may be passed
to device-callable routines from within \ac{GPU} kernels. Device contexts shall
not be created or destroyed from within \ac{GPU} kernels.
...
with \VAR{ctx}, or \CONST{SHMEM\_DEVICE\_TEAM\_WORLD} otherwise.}
\end{apiarguments}
...
Routines: \FUNC{shmemg\_atomic\_set}, \FUNC{shmemg\_atomic\_add},
\FUNC{shmemg\_atomic\_inc}, \FUNC{shmemg\_atomic\_and},
\FUNC{shmemg\_atomic\_or}, \FUNC{shmemg\_atomic\_xor}, and their
\texttt{shmemg\_ctx\_*} variants.
...
Routines: \FUNC{shmemg\_atomic\_fetch}, \FUNC{shmemg\_atomic\_swap},
\FUNC{shmemg\_atomic\_compare\_swap}, \FUNC{shmemg\_atomic\_fetch\_add},
\FUNC{shmemg\_atomic\_fetch\_inc}, \FUNC{shmemg\_atomic\_fetch\_and},
\FUNC{shmemg\_atomic\_fetch\_or}, \FUNC{shmemg\_atomic\_fetch\_xor}, and their
\texttt{shmemg\_ctx\_*} variants.
...
\hline
\FUNC{shmemg\_atomic\_compare\_swap} & Compares value at \VAR{dest} with \VAR{cond}; if equal, replaces with \VAR{value}. Returns the prior value. \\
\hline
...
Routines: \FUNC{shmemg\_atomic\_fetch\_nbi}, \FUNC{shmemg\_atomic\_swap\_nbi},
\FUNC{shmemg\_atomic\_compare\_swap\_nbi},
\FUNC{shmemg\_atomic\_fetch\_add\_nbi},
\FUNC{shmemg\_atomic\_fetch\_inc\_nbi},
\FUNC{shmemg\_atomic\_fetch\_and\_nbi},
\FUNC{shmemg\_atomic\_fetch\_or\_nbi},
\FUNC{shmemg\_atomic\_fetch\_xor\_nbi}, and their \texttt{shmemg\_ctx\_*}
variants.
...
\apidescription{
The routines initiate a copy and signal update. The routines return after
initiating the operation. The operation is considered complete after a subsequent
...
put-with-signal operations issued from within a \ac{GPU} device kernel. All
memory ordering routines operate at single-thread granularity.
...
\begin{apiarguments}
\apiargument{IN}{team}{The team over which to perform the synchronization.}
\end{apiarguments}

\apidescription{
\FUNC{shmemg\_team\_sync} is a collective synchronization routine over an
existing \openshmem team. The routine blocks until all \acp{PE} in the specified
team have called \FUNC{shmemg\_team\_sync}. All \acp{PE} in the team shall
participate.
...
\item Using a context associated with a team other than
  \CONST{SHMEM\_DEVICE\_TEAM\_WORLD} or a team derived from it for GPU-centric
  operations.
\item Passing an invalid device context handle to a GPU-centric routine.
...
\item Defined GPU-centric \ac{RMA}, \ac{AMO}, signaling, point-to-point
  synchronization, memory ordering, and collective operations with thread, thread
  group, and hardware thread group granularity.
