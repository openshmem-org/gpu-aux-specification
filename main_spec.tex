...
specification. In this version of the specification, every \ac{PE} in the
\openshmem program shall be associated with exactly one \ac{GPU}. The association
...
\ac{GPU}. Every \ac{PE} in the \openshmem program shall have a valid PE-to-GPU
association; a program in which any \ac{PE} lacks a valid PE-to-GPU association
is not a conforming program under this specification. Because every \ac{PE} has a
valid PE-to-GPU association, all \acp{PE} may invoke the \ac{GPU} interfaces
defined herein.

\textbf{Note:} This version of the specification defines a homogeneous program
model in which every \ac{PE} is associated with a \ac{GPU}. Support for
heterogeneous programs, in which some \acp{PE} do not have a PE-to-GPU
association, may be considered in a future version of the specification.
...
Because every \ac{PE} has a valid PE-to-GPU association (see
\cref{subsec:pe_gpu_association}), the set of member \acp{PE} in
\CONST{SHMEM\_DEVICE\_TEAM\_WORLD} is equal to the set of member \acp{PE} in
\CONST{SHMEM\_TEAM\_WORLD}. \CONST{SHMEM\_DEVICE\_TEAM\_WORLD} nonetheless remains
a separate predefined team from \CONST{SHMEM\_TEAM\_WORLD}; for example, it forms
a distinct atomicity domain (see \cref{subsec:atomicity_domains}) even though its
...
\acp{PE} in the \openshmem program. Because every \ac{PE} has a valid PE-to-GPU
association, \CONST{SHMEM\_DEVICE\_TEAM\_WORLD} shall be equal to
